...
class EventInputHandler:
    QUESTIONS = {
        ...
        EventState.IMAGE: "🖼️ لطفاً تصویر رویداد را ارسال کنید (یا 'بدون تصویر' را بنویسید):",
    ...
    def generate_preview_message(cls, user_data: Dict[str, Any]) -> str:
        ...
        preview = f"""
📌 عنوان: {user_data.get('title', '')}
📝 توضیحات: {user_data.get('description', '')}
📅 تاریخ: {user_data.get('date', '')}
⏰ ساعت: {user_data.get('time', '')}
🔄 تکرار: {repeat_type}
🔔 یادآوری: {notify_text}
🖼️ تصویر: {'دارد' if user_data.get('image') else 'ندارد'}
"""
        return preview
